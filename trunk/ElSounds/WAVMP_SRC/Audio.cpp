...
void StartDX()
{
    ...
    */
    ...
    if (hr == DS_OK)
    {
        Timer = timeSetEvent(floor(Args->BufferSize * Args->AudioBits /  8 * 1000 / Args->SampleRate / Args->Channels), 0,  &TimeProcDX, 0, TIME_ONESHOT | TIME_CALLBACK_FUNCTION);
        if (!Timer) 
        {
            int i = GetLastError();
            LastError = peOutputError;           
...
void CALLBACK TimeProcDX(UINT uID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2)
{
    ...
    {   // All of sound not played yet, send more data to buffer
        ...
        Timer = timeSetEvent(floor(Args->BufferSize * Args->AudioBits /  8 * 1000 / Args->SampleRate / Args->Channels) - 1, 0,  TimeProcDX, 0, TIME_ONESHOT | TIME_CALLBACK_FUNCTION);
    }
    else
    {
...
bool Audio_SetVolume(unsigned long Volume) {
    ...
    if (Args->OutputMode == omDirectSound) {
        if (DSBuffer) 
        {
            ...
            X = (-10000/log10(1/65536))*log10((X + 1)/65536);
...
bool Audio_GetVolume(unsigned long *Volume) {
    ...
    if (Args->OutputMode == omDirectSound) {
        if (DSBuffer) {
            ...
            *Volume = 65536*pow(10,(*Volume/(-10000/log10(1/65536))) - 1);
